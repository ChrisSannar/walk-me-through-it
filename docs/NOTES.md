...
### MVP
 ...
 - Allow users to write and select their own `wmti_*.json` files and run them in the project (write and provide a style guide so that an LLM can generate them. Is that a "Skill"?)
...
## Engineering
 ...
 - Maybe keep all the `wmti_*.json` files somewhere in the local storage file so they don't clutter things up. (also include an option to store them locally if the project is to be shared)
